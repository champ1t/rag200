...
TEST_CASES = [
    {
        ...
        "expect_in_answer": ["เนื้อหามีรายละเอียดเพิ่มเติม", "อ่านต่อฉบับเต็มได้ที่"],
        "expect_not_in_answer": ["1. | 133"] # Should NOT show the raw table index list
    },
    {
        "name": "Ribbon Directory",
        "query": "Ribbon EdgeMare6000 DOC",
        "expect_in_answer": ["EdgeView_16.0.1_User_Guide"],
        "min_answer_len": 50
    },
    {
        ...
        "expect_not_in_answer": ["| 51 | 123"] # Should be cleaned or summarized
    },
    {
        "name": "JSON Sanitization (Phuket)",
        "query": "เบอร์ สื่อสารข้อมูล ภูเก็ต",
        "expect_valid_json": True, # Implied by requests success
    ...
    },
    {
        ...
        "check_route": "article_answer" # or similar
    }
]

def run_tests():
    print("=== Running 5 Verification Tests ===")
    results = []
