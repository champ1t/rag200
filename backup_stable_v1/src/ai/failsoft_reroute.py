...
def should_reroute_to_howto(query: str, route: str) -> bool:
    """
    Deterministic check for fail-soft reroute eligibility.
    
    Args:
        query: The normalized query
        route: The current route (should be "contact_miss_strict" or "team_miss")
        
    Returns:
        True if query should be rerouted to HOWTO_PROCEDURE
    """
    # Only reroute on contact/team miss
    if route not in ["contact_miss_strict", "team_miss"]:
        return False
    ...
    return has_tech_entity and has_config_verb
