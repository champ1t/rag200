...
class SMCStressTest(unittest.TestCase):
    ...
    def test_run_50_questions(self):
        ...
        questions = [
            ...
            ("SMC Extension Dashboard คือลิงก์ไหน", ["link_lookup", "article_answer"], ["192.168.99.200"]),
            ...
            ("ลิงก์ NT Academy", ["link_lookup"], ["lms.ntplc"]),
            ("เว็บกองทุนสำรองเลี้ยงชีพ", ["link_lookup"], ["pvd"]),
            ("ระบบสุราษฎร์ NMS", ["link_lookup", "article_answer"], ["totsni"]),
            ("ระบบนราธิวาส NMS", ["link_lookup", "article_answer"], ["10.192.39.50"]),
