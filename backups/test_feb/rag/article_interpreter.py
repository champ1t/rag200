...
class ArticleInterpreter:
    ...
    def interpret(self, user_query: str, article_title: str, article_url: str, article_content: str, images: list = None, show_images: bool = False, match_score: float = 0.0, intent: str = None) -> str:
        ...
        CACHE_SCHEMA_VERSION = "v240_broad_knowledge"
        ...
        result = self._interpret_logic(user_query, article_title, article_url, article_content, images, show_images, match_score, intent=intent, pre_cleaned_content=cleaned_content)
        
        # Cache Result
        self._result_cache[cache_key] = result
        return result

    def _interpret_logic(self, user_query: str, article_title: str, article_url: str, article_content: str, images: list = None, show_images: bool = False, match_score: float = 0.0, intent: str = None, pre_cleaned_content: str = None) -> str:
        """
        ...
        if not article_content or len(article_content.strip()) < 10:
             return f"เนื้อหาในบทความสั้นเกินไปหรือไม่พบข้อมูล\n\nแหล่งที่มา:\n🔗 {article_url}"
        ...
        if pre_cleaned_content:
             ...
             # Fallback (Should not happen ideally if called from wrapper)
             from src.rag.article_cleaner import clean_article_content, strip_menus, deduplicate_paragraphs
             cleaned_content = clean_article_content(article_content, keep_metadata=True)
             cleaned_content = strip_menus(cleaned_content)
             cleaned_content = deduplicate_paragraphs(cleaned_content)
...
"""
            try:
                ...
                # Append Footer (Source Standardization)
                llm_summary += f"\n\nแหล่งที่มา:\n🔗 {article_url}"
                timings["total_ms"] = (time.time() - t_start) * 1000
                return llm_summary
        ...
        if is_tutorial_mode and False: # DISABLED Phase 185: merged into Phase 198 Hybrid Strategy
            ...
            # B) Long Article (>= 1200 chars) -> LLM Summarizer (High Quality)
                ...
                try:
                    ...
                    # Phase 219: Python-side Guardrails
        ...
        if not is_tutorial_mode:
            ...
            if is_metadata or is_nav:
                ...
                if is_nav:
                    ...
                    if valid_links:
                         links_str = "\n".join(valid_links)
                         return f"เนื้อหานี้เป็นเพียงหน้ารวมลิงก์/เมนู (Navigation Page) กรุณาเลือกหัวข้อที่ต้องการ:\n\n{links_str}"
                    
                    return "เนื้อหาที่พบเป็นเพียงเมนูนำทาง ไม่พบข้อมูลรายละเอียดในบทความนี้"
                    
                return "ไม่พบข้อมูลที่ยืนยันคำตอบนี้ในระบบ"
        ...
        if self._looks_like_link_directory(cleaned_content, user_query):
             print(f"[ArticleInterpreter] Detected Link Directory/Manual Page")
             directory_ans = self._parse_link_directory(cleaned_content, article_title, user_query, article_url)
             if directory_ans:
                 timings["total_ms"] = (time.time() - t_start) * 1000
                 return directory_ans
...
If images are relevant to the user's question, include them in a separate section at the end."""
    ...
    def _parse_link_directory(self, content: str, title: str, user_query: str = "", article_url: str = "") -> str:
        ...
        for item in found_links:
            ...
            if any(k in u_lower for k in junk_patterns) or any(k in t_lower for k in junk_patterns): 
                continue
        ...
        # Phase 240: Broad Knowledge Query Detection
        broad_keywords = ["มีอะไรบ้าง", "ความรู้ทั่วไป", "ภาพรวม", "overview", "list of topics", "general knowledge"]
        is_broad_query = any(k in user_query.lower() for k in broad_keywords)
        
        if is_broad_query:
            # Provide structured general knowledge answer
            out_lines = [f"[{display_title}]"]
            out_lines.append("\nหัวข้อที่เกี่ยวข้อง:\n")
            for item in top_links:
                topic = item['text'].strip()
                out_lines.append(f"• {topic}")
            out_lines.append("\n📌 สามารถดูรายละเอียดเพิ่มเติมได้จากเอกสารอ้างอิง")
            out_lines.append(f"\nแหล่งที่มา:\n🔗 {article_url}")
            return "\n".join(out_lines).strip()
        
        # Original article-based framing for specific queries
