...
class ChatEngine:
    ...
    def process(self, q: str, session_id: str = "default") -> Dict[str, Any]:
        ...
        # Guard D: Greeting Gate (Phase 97 Check)
        if is_pure_greeting(q_clean_for_guard):
             greeting_res = get_greeting_response()
             if greeting_res:
                 print(f"[CHAT] Greeting Gate Hit: {q}")
                 return {
                     "answer": greeting_res,
                     "route": "greeting_gate",
                     "latencies": latencies
        ...
        if intent == "CONTACT_LOOKUP":
            if not is_valid_contact_query(q):
                ...
                should_reroute, new_intent = should_reroute_to_howto(q, intent)
                if should_reroute:
        ...
        if intent in ["HOWTO_PROCEDURE", "THAI_ORG_KNOWLEDGE"]:
             ...
             if hits:
                 for h in hits:
                     ...
                     if h.score > 0.35: # Phase 237: Lowered to 0.35 for Summary, 0.20-0.35 falls back to Link-Only RAG
        ...
        if intent == "CONTACT_LOOKUP":
            ...
            if res.get("route") in ["contact_ambiguous", "contact_broad_list", "contact_ambiguous_all"]:
                 candidates = []
                 try:
            ...
            if res.get("hits"):
                hits = res["hits"]
                self.last_context = {"type": "contact", "data": hits[0], "ref_name": hits[0].get("name")}
        ...
        # GUARDRAL: Bypass clarification for Informational Intents (Fix for "ONU คืออะไร" -> asking about Wi-Fi)
        INFORMATIONAL_INTENTS = ["DEFINE_TERM", "EXPLAIN", "SUMMARY", "COMPARE", "WHAT_IS", "CONCEPT_EXPLAIN", "FIELD_CONCEPT_REASONING"]
        is_info_intent = intent in INFORMATIONAL_INTENTS
        
        # Also check for "What Is" pattern in query if intent classifier missed it
        if not is_info_intent:
             if any(k in q.lower() for k in ["คืออะไร", "หมายถึง", "definition", "concept"]):
                  is_info_intent = True
        ...
        if valid_results and gen_intent in ["HOWTO_PROCEDURE", "EXPLAIN", "SUMMARIZE", "CONCEPT_EXPLAIN"]:
            ...
            if top_score < 0.35:
                # Phase 240: Conceptual Explain Mode Detection
                # Check if this is a principle-based/general knowledge question
                conceptual_indicators = [
                    "หลักการ", "โดยทั่วไป", "อธิบาย", "ทำงานยังไง", 
                    "ช่วยวิเคราะห์", "principle", "generally", "how does it work"
                ]
                is_conceptual = any(ind in q.lower() for ind in conceptual_indicators)
                
                # Only activate if score is reasonable (0.25-0.35) and question is conceptual
                if is_conceptual and top_score >= 0.25:
                    print(f"[DEBUG] Conceptual Explain Mode: Score {top_score:.4f} (0.25-0.35), Conceptual Query Detected")
                    
                    # Use LLM to provide general explanation
                    from src.rag.prompts import TEMPLATE_CONCEPTUAL_EXPLAIN
                    
                    # Get context from top results
                    context_parts = []
                    for hit in valid_results[:2]:  # Use top 2 results
                        text = getattr(hit, "page_content", "")
                        if text:
                            context_parts.append(text[:1500])  # Limit each to 1500 chars
                    
                    context_str = "\n\n---\n\n".join(context_parts)
                    
                    try:
                        prompt = TEMPLATE_CONCEPTUAL_EXPLAIN.format(
                            context_str=context_str,
                            query=q
                        )
                        
                        llm_answer = ollama_generate(
                            base_url=self.llm_cfg.get("base_url", "http://localhost:11434"),
                            model=self.llm_cfg.get("fast_model", "llama3.2:3b"),
                            prompt=prompt,
                        ...
                        latencies["llm"] = (time.time() - t_start) * 1000 - latencies.get("vs", 0)
                        latencies["total"] = (time.time() - t_start) * 1000
                        
                        return {
                            "answer": llm_answer,
                            "route": "rag_conceptual_explain",
                            "latencies": latencies,
                            "score": top_score
                        }
                    except Exception as e:
                        print(f"[DEBUG] Conceptual Explain Mode failed: {e}. Falling back to Link-Only.")
                        # Fall through to Link-Only mode below
    ...
    def _handle_article_route(self, url: str, query: str, latencies: Dict[str, float], start_time: float, match_score: float = 0.0, intent: str = None) -> Dict[str, Any]:
        """
        ...
        if cm:
            ...
            if l2_hit:
                 ...
                 return {
                     "answer": self.normalize_smc_url(clean_junk_text(l2_hit["answer"])),
                     "route": "article_answer", # or article_cache
                     "latencies": latencies,
                     "hits": [{"key": "Source", "value": "News", "source_url": self.normalize_smc_url(url)}]
                 }
        ...
        ans_data = self.article_interpreter.interpret(
            ...
            intent=intent # Pass intent to trigger override
        )
        ...
        # Phase 110: Junk Filter (Rule JR-1) + URL Normalization
