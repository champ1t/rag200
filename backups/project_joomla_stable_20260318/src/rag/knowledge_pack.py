...
class KnowledgePackManager:
    ...
    def lookup(self, query: str) -> Optional[Dict[str, Any]]:
        ...
        # Huawei NE8000 command lookup
        _thai_cmd_hints = [
            "อัตราการใช้", "หน่วยความจำ", "routing-table", "routing table",
            "serial number", "manuinfo", "cpu-usage",
        ]
        if (any(w in q_lower for w in ["huawei", "ne8000", "display "])
                or any(w in q_lower for w in _thai_cmd_hints)):
            target_cats.append("huawei_ne8000_cmd")
        ...
        for cat in target_cats:
            ...
            for fact in facts:
                ...
                elif fact["category"] == "huawei_ne8000_cmd":
                    cmd_key = fact["key"].lower()
                    val_lower = fact.get("value", "").lower()
                    # Exact command name in query  → highest score
                    if cmd_key in q_lower:
                        score += 15
                    else:
                        # Partial word match: each word of command found in query
                        cmd_words = [w for w in cmd_key.split() if len(w) > 2]
                        matched_words = sum(1 for w in cmd_words if w in q_lower)
                        if matched_words >= 2:
                            score += matched_words * 3
                        elif matched_words == 1:
                            score += 1
                    # Bonus: Thai description keywords in query
                    desc_words = [w for w in val_lower.split() if len(w) >= 3 and '\u0e00' <= w[0] <= '\u0e7f']
                    for dw in desc_words[:5]:
                        if dw in q_lower:
                            score += 4
                    # Bonus: English phrase in description value found in query
                    desc_ascii_words = [w for w in val_lower.split() if len(w) >= 4 and w.isascii() and w.isalpha()]
                    # Use set for unique matching only
                    for aw in set(desc_ascii_words):
                        if aw in ["huawei", "ne8000"]: continue # Skip vendor/model common words
                        if aw in q_lower:
                            score += 5
                    # Base: generic huawei/ne8000 query → include all commands (low score)
                    if "ne8000" in q_lower or ("huawei" in q_lower and "display" in q_lower):
                        score = max(score, 1) # Ensure at least score 1 if matched
                    
                    # DEBUG TRACE within loop
                    if "cpu-usage" in cmd_key: # Limit noise
                         print(f"[DEBUG_KP_SCORE] Matched Huawei Cmd: {cmd_key} | Score={score}")
        ...
        for fact, score in hits:
            ...
            # Create a shallow copy to inject runtime metadata without mutating source
            fact_copy = fact.copy()
            fact_copy["_score"] = score
            unique_hits.append(fact_copy)
