...
class ChatEngine:
    ...
    def _classify_intent(self, query: str) -> str:
        ...
        Returns: 'COMMAND', 'CONFIG', 'PROTOCOL', 'OVERVIEW', 'TROUBLESHOOT', or 'UNKNOWN'
        """
        q_lower = query.lower()
        ...
        # Priority Order: COMMAND > CONFIG > PROTOCOL > OVERVIEW > TROUBLESHOOT
        keywords = {
            "COMMAND": ["command", "show ", "display ", "cmd"],
            "CONFIG": ["config", "cli", "ตั้งค่า", "configure", "setup", "manual", "คู่มือ"],
            "PROTOCOL": ["bgp", "ospf", "isis", "mpls", "vlan", "vxlan", "evpn"],
            "OVERVIEW": ["overview", "spec", "คืออะไร", "what is", "introduction", "feature"],
            "TROUBLESHOOT": ["error", "problem", "fail", "ปัญหา", "แก้", "debug", "logs", "monitor"]
        }
    ...
    def _is_article_compatible(self, intent: str, article_type: str, query: str, is_exact: bool = False) -> bool:
        ...
        if intent in ["COMMAND", "CONFIG"]:
            ...
            if article_type in ["COMMAND_REFERENCE", "CONFIG_GUIDE", "FEATURE_USAGE"]:
                return True
    ...
    def _check_out_of_scope(self, query: str) -> bool:
        ...
        q_lower = query.lower()
        block_patterns = [
            "opinion", "comment", "recommend", "best", "better", "worse", # Opinions
            "compare", "vs ", "versus", "difference", # Comparisons
            "why ", "reason for" # Open-ended "Why" often leads to hallucination
        ]
        
        # Exception: "Reason for error" is valid troubleshooting
        if "error" in q_lower or "fail" in q_lower:
            return False
            
        return any(p in q_lower for p in block_patterns)
    ...
    def _is_smc_url(self, url: str) -> bool:
        ...
        Only SMC URLs (INTERNAL_SMC_IP) are considered trusted internal sources.
        """
        if not url:
            return False
        
        url_lower = url.lower()
        smc_patterns = ["INTERNAL_SMC_IP", "smc"]
    ...
    def _process_logic(self, q: str, session_id: str = "default") -> Dict[str, Any]:
        ...
        # Check for direct article matches BEFORE any other logic
        try:
             ...
             if det_hit:
                 ...
                 if det_hit.get("match_type") == "deterministic":
                     ...
                     if check_compatibility and not self._is_article_compatible(query_intent, art_type, q, is_exact=is_exact):
                         ...
                         if v_scope.get('vendor') and v_scope.get('type') == 'SMC_ONLY':
    ...
    def _handle_article_route(self, url: str, query: str, latencies: Dict[str, float], start_time: float, match_score: float = 0.0, intent: str = None, article_type: str = None, decision_reason: str = None) -> Dict[str, Any]:
        ...
        if hasattr(self, 'processed_cache') and self.processed_cache:
            ...
            # Simplify: Check if URL contains trusted domain (INTERNAL_SMC_IP) OR is in cache
            if "INTERNAL_SMC_IP" in url or "smc" in url.lower():
                is_verified = True
