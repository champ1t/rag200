...
# Colloquial Pattern Dictionary for Thai Query Normalization
COLLOQUIAL_PATTERNS = {
    # Intent phrases (map to standard query format)
    r'อยากติดต่อ\s*': 'ขอเบอร์ ',
    r'อยากรู้เบอร์\s*': 'ขอเบอร์ ',
    r'บอกเบอร์\s*': 'ขอเบอร์ ',
    r'ช่วยบอกเบอร์\s*': 'ขอเบอร์ ',
    
    # Question normalization (colloquial → standard)
    r'ทำงานยังไง': 'ทำงานอย่างไร',
    r'ใช้ยังไง': 'ใช้อย่างไร',
    r'ทำยังไง': 'ทำอย่างไร',
    
    # Emphatic removal (already handled by ColloquialNoiseRemover, but backup)
    r'\s+มันคืออะไร': ' คืออะไร',
    r'\s+มัน\s+': ' ',
}

# Compile patterns for performance
COMPILED_COLLOQUIAL_PATTERNS = [
    (re.compile(pattern), replacement)
    for pattern, replacement in COLLOQUIAL_PATTERNS.items()
]
...
COLLOQUIAL PATTERNS (recognize and normalize):
- "อยากติดต่อ X" → "ขอเบอร์ X"
- "บอกเบอร์ X" → "ขอเบอร์ X"
- "X ทำงานยังไง" → "X ทำงานอย่างไร"
- "X มันคืออะไร" → "X คืออะไร"
...
Input: "อยากติดต่อ CSOC"
Output: "ขอเบอร์ CSOC"

Input: "OLT ทำงานยังไง"
Output: "OLT ทำงานอย่างไร"
...
class QueryNormalizer:
    ...
    def _apply_colloquial_patterns(self, query: str) -> Tuple[str, bool]:
        """
        Apply colloquial pattern mappings to normalize Thai queries.
        
        Args:
            query: Original query
            
        Returns:
            (normalized_query, was_modified)
        """
        original = query
        
        for pattern, replacement in COMPILED_COLLOQUIAL_PATTERNS:
            query = pattern.sub(replacement, query)
        
        # Clean up multiple spaces
        query = re.sub(r'\s+', ' ', query).strip()
        
        changed = (query != original)
        if changed:
            print(f"[ColloquialPatterns] '{original}' → '{query}'")
        
        return query, changed
    
    def normalize(self, query: str, trigger_reason: str = "deterministic_miss") -> Dict[str, Any]:
        """
        Normalize a query using colloquial patterns + LLM paraphrasing.
        ...
        # Step 1: Apply colloquial patterns FIRST (fast, deterministic)
        query, pattern_applied = self._apply_colloquial_patterns(query)
        
        # Step 2: Check if LLM normalization needed
        if self._is_already_canonical(query):
            return {
                "normalized_query": query,
                "changed": pattern_applied,
                "confidence": 1.0,
                "latency_ms": 0,
                "trigger": "colloquial_patterns_only" if pattern_applied else "skip_already_canonical"
            }
        ...
        try:
            ...
            changed = (normalized.lower() != query.lower()) or pattern_applied
            
            if changed and normalized.lower() != query.lower():
            ...
            return {
                "normalized_query": query,
                "changed": pattern_applied,  # Still report pattern changes even if LLM fails
                "confidence": 0.0,
                "latency_ms": 0,
                "trigger": "error"
            }
